...
describe('data_generator purchaseByDate', () => {
  ...
  it('필요한 각 항목이 모두 생성되는지 확인', () => {
    ...
    expect(purchase.filter).toHaveProperty([0, 'key'])
    expect(purchase.filter).toHaveProperty([0, 'title'])
    console.log(purchase.filter.find(x => x.sortable))
    console.log(purchase.filter.find(x => x.component))
    expect(purchase.filter.find(x => x.sortable)).not.toBe(undefined)
    expect(purchase.filter.find(x => x.component)).not.toBe(undefined)
  })
})